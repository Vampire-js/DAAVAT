...
export default function QuizGenerator({ isPopup = false }: { isPopup?: boolean }) {
  ...
  // Default collapsed if not a popup
  const [isMinimized, setIsMinimized] = useState(!isPopup);
  ...
  const handleGenerate = async () => {
    ...
    // 1. Extract text from the main editor note
    const noteText = extractTextFromBlocks(content || "");

    // 2. Extract and append content from all individual sources/references
    const sourcesText = (references || [])
      .map(ref => `--- Source: ${ref.title} (${ref.source}) ---\n${ref.content}`)
      .join("\n\n");

    // 3. Combine both for the AI to read
    const combinedContext = `Main Note Content:\n${noteText}\n\nSupporting Sources Content:\n${sourcesText}`;

    if (!combinedContext || combinedContext.trim().length < 50) {
        setError("Not enough content in notes or sources to generate a quiz.");
        setLoading(false);
        return;
    }
  ...
  return (
    <div className={clsx(
        ...
        isPopup ? "h-[80vh]" : (isMinimized ? 'h-auto' : 'h-[500px]'),
        !isPopup && "rounded-xl border border-neutral-800 shadow-2xl backdrop-blur-md"
      )}>
      
      {/* Header Card */}
      <div 
        className={clsx(
            "p-4 bg-neutral-900/80 border-b border-neutral-800 flex items-center justify-between transition-colors",
            !isPopup && "cursor-pointer hover:bg-neutral-800/80 rounded-t-xl"
        )}
        onClick={() => !isPopup && setIsMinimized(!isMinimized)}
      >
        <div className="flex items-center gap-3">
            ...
            <div>
                <h3 className="font-bold text-white text-sm md:text-base">Interactive Quiz</h3>
            </div>
        </div>
        ...
        <div className="flex items-center gap-2">
            {(!isMinimized || isPopup) && (
                <Button 
                    ...
                    size="sm" 
                    className={clsx(
                        "h-8 text-xs border border-transparent font-medium",
                        quizData 
                            ? "bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white" 
                            : "bg-indigo-600 hover:bg-indigo-700 text-white"
                    )}
                >
                    {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : (quizData ? "Regenerate" : "Start Quiz")}
                </Button>
            )}
